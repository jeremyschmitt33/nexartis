...
export default function ModifierFacturePage() {
  ...
  useEffect(() => {
    ...
    // V3.0c.17 — Type + champs situation
    if (facture.type === 'standard' || facture.type === 'acompte' || facture.type === 'situation' || facture.type === 'avoir') {
      setFactureType(facture.type)
    }
    if (facture.devis_ref) setDevisRef(facture.devis_ref)
    if (facture.numero_situation && facture.numero_situation > 0) setNumeroSituation(facture.numero_situation)
    if (facture.pourcentage_situation != null) setPourcentageSituation(facture.pourcentage_situation)
      ...
      <div className="min-h-screen flex items-start justify-center px-4 py-12">
        <div className="relative w-full max-w-2xl bg-white rounded-3xl border border-[#0f1a3a]/[0.06] p-8 overflow-hidden shadow-[0_8px_24px_rgba(15,26,58,0.06),_0_1px_4px_rgba(15,26,58,0.04)]">
          ...
          <div className="flex items-start gap-4 mb-6">
            <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-[#ff7a1a] to-[#ff9d4d] flex items-center justify-center text-white shadow-[0_8px_20px_rgba(255,122,26,0.35),_inset_0_1px_0_rgba(255,255,255,0.25)]">
              <Lock size={22} />
            </div>
            <div className="min-w-0">
              <h1 className="font-hanken font-extrabold text-xl sm:text-2xl text-[#0f1a3a] tracking-[-0.025em]">
                Facture <span className="font-spline-mono font-medium tracking-[0.5px]">{facture.numero}</span> verrouillée
              </h1>
              <p className="font-hanken text-sm text-gray-500 mt-1.5">
                Statut actuel : <span className="font-semibold capitalize text-[#0f1a3a]">{facture.statut}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 h-11 px-5 rounded-xl bg-gradient-to-br from-[#ff7a1a] to-[#ff9d4d] text-white font-hanken text-[13.5px] font-bold shadow-[0_6px_16px_rgba(255,122,26,0.30),_inset_0_1px_0_rgba(255,255,255,0.25)] opacity-60 cursor-not-allowed"
            >
  ...
  return (
    <div className="min-h-screen">
      {/* Top bar V4 light : sticky en haut, fond blanc, séparateur subtil, CTA orange à droite. */}
      <div className="sticky top-0 bg-white border-b border-[#0f1a3a]/[0.06] z-10 py-3 px-6 flex items-center justify-between backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <Link href={'/dashboard/factures/' + id} className="p-2 rounded-xl hover:bg-[#fafbfc] transition-colors" aria-label="Retour">
            <ArrowLeft size={18} className="text-gray-600" />
          </Link>
          <h2 className="hidden sm:block font-hanken font-extrabold text-lg text-[#0f1a3a] tracking-[-0.025em]">
            Modifier <span className="font-spline-mono font-medium tracking-[0.5px]">{facture.numero}</span>
          </h2>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={'/dashboard/factures/' + id}
            className="h-9 px-3 rounded-xl border-[1.5px] border-gray-200 bg-white font-hanken text-[13px] font-semibold text-[#0f1a3a] hover:border-[#ff7a1a] hover:bg-[#fafbfc] transition-all inline-flex items-center"
          >
            Annuler
          </Link>
          <button
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Carte Dates + Objet — V4 light premium */}
          <div className="relative bg-white rounded-3xl border border-[#0f1a3a]/[0.06] p-6 sm:p-8 space-y-4 overflow-hidden shadow-[0_8px_24px_rgba(15,26,58,0.06),_0_1px_4px_rgba(15,26,58,0.04)]">
            ...
            <div className="relative">
              ...
              <p className="mt-1.5 font-hanken text-xs text-gray-500">
                Tapez pour rechercher un chantier existant, ou saisissez librement.
              </p>
            </div>
        ...
        <div className="relative bg-white rounded-3xl border border-[#0f1a3a]/[0.06] overflow-hidden shadow-[0_8px_24px_rgba(15,26,58,0.06),_0_1px_4px_rgba(15,26,58,0.04)]">
          ...
          <div className="sm:hidden p-3 space-y-2">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {/* CTA principal mobile : ajouter une ligne (gradient orange) */}
              <button
                type="button"
          ...
          {/* Desktop : table grille avec 7 colonnes (designation, qté, unité, prix HT, TVA, total, supprimer). */}
          <div className="hidden sm:block overflow-x-auto">
            ...
            {lines.map((line, idx) => {
              // Section : bandeau orange clair, designation en gras uppercase
              ...
              // Sous-section : bandeau plus clair
          ...
          {/* Boutons "ajouter" desktop — CTA principal orange, sections en outline */}
        ...
        {/* Totaux — récap V4 light, chiffres en Spline Sans Mono, NET À PAYER en gradient orange. */}
        <div className="flex justify-end">
          <div className="relative bg-white rounded-3xl border border-[#0f1a3a]/[0.06] p-6 w-full sm:w-96 overflow-hidden shadow-[0_8px_24px_rgba(15,26,58,0.06),_0_1px_4px_rgba(15,26,58,0.04)]">
            ...
            {/* Ventilation TVA par taux (multi-taux Obat) */}
            ...
            {/* Bloc NET À PAYER : gradient orange (signature CTA V4) */}
        ...
        {/* Boutons bas — actions de sauvegarde finales */}
        <div className="flex flex-wrap items-center gap-3 justify-end pb-8">
          <Link
            href={'/dashboard/factures/' + id}
            className="h-12 px-6 rounded-xl border-[1.5px] border-gray-200 bg-white font-hanken text-[13.5px] font-semibold text-[#0f1a3a] hover:border-[#ff7a1a] hover:bg-[#fafbfc] transition-all inline-flex items-center"
          >
            Annuler
          </Link>
          <button
            onClick={() => handleSave('brouillon')}
            disabled={saving}
            className="h-12 px-6 rounded-xl border-[1.5px] border-gray-200 bg-white font-hanken text-[13.5px] font-semibold text-[#0f1a3a] hover:border-[#ff7a1a] hover:bg-[#fafbfc] disabled:opacity-50 transition-all"
          >
          ...
          <button
            onClick={() => handleSave('enregistrer')}
            disabled={saving}
            className="h-12 px-8 rounded-xl bg-gradient-to-br from-[#ff7a1a] to-[#ff9d4d] text-white font-hanken text-[14.5px] font-bold shadow-[0_8px_20px_rgba(255,122,26,0.35),_inset_0_1px_0_rgba(255,255,255,0.25)] hover:-translate-y-0.5 hover:brightness-105 active:translate-y-0 disabled:opacity-50 disabled:hover:translate-y-0 transition-all"
          >
      ...
      {/* Bottom sheet mobile (composant existant — NON modifié) */}
      <LineSheet
        open={sheetOpen}
      ...
      {toastMsg && (
        <div className="fixed bottom-6 right-6 bg-[#0f1a3a] text-white px-4 py-2.5 rounded-xl shadow-xl font-hanken text-sm z-50">{toastMsg}</div>
      )}
    </div>
  )
}

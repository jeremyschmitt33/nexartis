...
const data = {
  ...
  longueIntro:
    "Choisir un logiciel de devis et factures quand on est plombier en 2026 n'a rien d'un détail. Entre les trois taux de TVA (5,5%, 10% et 20%) qui s'appliquent selon le type de travaux, la mention décennale obligatoire sur chaque document, la fin de l'attestation TVA papier au 16 février 2025, et l'arrivée prochaine de Factur-X qui imposera la facture électronique normalisée à toutes les entreprises, l'outil que vous utilisez chaque soir au bureau (ou chaque soir dans votre camion) doit suivre. Or, dans le métier, près de 70% des plombiers exercent en auto-entrepreneur ou en TPE solo. Beaucoup d'entre vous interviennent en astreinte, le dimanche à 22h, sur une fuite qui inonde une cuisine, et la dernière chose dont vous avez envie en remontant dans le camion, c'est de devoir taper un devis sur un ordinateur en rentrant. Nexartis a été conçu pour cette réalité : un logiciel français, pensé par et pour les artisans, qui édite un devis conforme depuis votre téléphone, applique le bon taux de TVA en deux clics, injecte automatiquement votre numéro de décennale, et prépare vos factures au format Factur-X sans que vous ayez à comprendre ce que cela signifie. Dans cet article, nous détaillons les trois taux de TVA en plomberie, les obligations réglementaires 2026, ce qu'un bon logiciel doit savoir faire — et comment Nexartis se positionne, à 15€ HT par mois pour le plan Essentiel.",
  ...
  tableauTva: [
    ...
    },
    {
      ...
      taux: "5,5%",
      conditions: "Rénovation énergétique + RGE",
    },
  ],
  ...
  conseilsRedaction: [
    ...
    "Détailler la main d'œuvre en heures et le tarif horaire HT, séparément du déplacement",
    "Préciser explicitement si l'enlèvement des anciens équipements est inclus, et si oui, à quel coût",
    "Indiquer les conditions de paiement : acompte demandé à la signature (souvent 30%), échéance facture (30 jours par défaut), taux de pénalités de retard",
  ...
  faqCustom: [
    {
      ...
      answer:
        "Le bon logiciel pour un plombier doit gérer les trois taux de TVA (5,5%, 10%, 20%), injecter automatiquement la mention décennale et le numéro d'assurance, être compatible Factur-X 2026, et permettre de créer un devis depuis le terrain en moins de 2 minutes. Le marché propose plusieurs options : Tolteck (19-25€/mois) reste simple et largement adopté, Obat (25-79€/mois) ajoute la signature électronique et BatiChiffrage, Henrri est gratuit mais pas spécifique BTP. Nexartis se positionne à 15€ HT/mois pour le plan Essentiel, conçu spécifiquement pour les artisans solo et auto-entrepreneurs.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "Oui. La réforme française impose Factur-X en réception au 1er septembre 2026 pour toutes les entreprises, et en émission au 1er septembre 2027 pour les TPE et auto-entrepreneurs. Toutes les factures Nexartis sont générées au format Factur-X (un PDF lisible avec un fichier XML structuré embarqué à l'intérieur), conformes au standard européen EN 16931. Vous n'avez aucune manipulation à faire : le format s'applique par défaut sur l'ensemble des factures émises depuis votre compte.",
    },
    {
      question: "Comment facturer rapidement un dépannage d'urgence ?",
      answer:
        "Nexartis est une PWA (application web installable sur iPhone et Android, sans passer par les stores). Depuis votre téléphone, vous créez la facture en direct chez le client en 1 à 2 minutes : sélection du client (ou création rapide), saisie des prestations avec les majorations nuit ou week-end pré-configurées dans votre bibliothèque, validation. La facture part par email avec un lien de paiement Stripe. Le client peut régler en CB depuis son téléphone avant même que vous soyez rentré au camion, et l'encaissement est enregistré automatiquement dans votre tableau de bord.",
    },
    {
      question:
        "Puis-je intégrer ma mention décennale automatiquement sur mes documents ?",
      answer:
        "Oui. Vous renseignez une seule fois dans les paramètres : le nom de votre assureur (AXA, MAAF, SMABTP, Groupama, etc.), le numéro de contrat, et la zone géographique de couverture. Ces informations apparaissent ensuite sur 100% de vos devis et factures, dans le pavé légal en bas de document, conformément aux exigences de la loi Spinetta. Si vous changez d'assureur ou renouvelez votre contrat, vous modifiez l'information à un seul endroit et tous les futurs documents s'actualisent.",
    },
    {
      question: "Le logiciel gère-t-il les acomptes et factures de situation ?",
      answer:
        "Oui. Vous pouvez exiger un acompte à la signature du devis (le standard en plomberie est de 30%, mais vous fixez le pourcentage). Pour les gros chantiers (rénovation complète de salle de bains, installation d'une PAC), vous facturez ensuite des situations intermédiaires : par exemple 30% à la signature, 40% à la pose, 30% à la mise en service. Le solde est calculé automatiquement à chaque étape, et chaque facture est rattachée au chantier d'origine pour conserver la traçabilité comptable.",
    },
    {
      question: "Comment créer un devis depuis le chantier ?",
      answer:
        "L'application est installable comme une vraie app sur iOS et Android. Vous l'ouvrez depuis l'icône sur l'écran d'accueil de votre téléphone, sans avoir besoin de navigateur. Le devis se crée en quelques tapotements : client (avec recherche par nom), prestations depuis votre bibliothèque (chauffe-eau, débouchage, déplacement nuit, etc.), validation. Le client signe directement avec le doigt sur l'écran de votre téléphone, et le PDF signé est envoyé par email. L'ensemble fonctionne même avec une connexion 4G faible, et synchronise automatiquement dès le retour en zone couverte.",
    },
    {
      question: "Que se passe-t-il après les 14 jours d'essai gratuit ?",
      answer:
        "Aucun prélèvement automatique : nous ne demandons pas de carte bancaire à l'inscription. À l'issue des 14 jours, vous choisissez de souscrire au plan Essentiel (15€ HT/mois, le minimum légal en BTP) ou au plan Complet (25€ HT/mois, qui inclut le planning, la commande vocale et la gestion d'équipe). Si vous ne souscrivez pas, votre compte reste accessible en lecture seule : vous gardez l'accès à vos documents déjà créés. Vous pouvez revenir à tout moment, sans perdre vos données.",
    },
  ],

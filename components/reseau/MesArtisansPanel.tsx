...
// ============================================================================
// components/reseau/MesArtisansPanel.tsx — Vue « Mes artisans » de la messagerie.
// ----------------------------------------------------------------------------
// Le réseau (confrères ajoutés, demandes reçues, invitations envoyées) est
// désormais intégré DANS la messagerie, dans le panneau de gauche, à côté de la
// vue « Discussions ». Un clic sur un artisan ouvre (ou crée) le fil de
// discussion : c'est le parent (messagerie) qui gère l'ouverture via
// `onOuvrirChat`. Aucune navigation de page ici.
//
// Repris de l'ancienne page « Mon réseau » (retirée du menu), adapté au format
// étroit du panneau. Toute la sécurité reste en base (RLS + RPC).
// ============================================================================

import { useMemo, useState } from 'react'
import { useContacts, type Contact } from '@/lib/hooks-messagerie'
import {
...
type SousOnglet = 'contacts' | 'demandes' | 'envoyees'

export default function MesArtisansPanel({ onOuvrirChat }: { onOuvrirChat: (userId: string) => Promise<void> }) {
  ...
  const [onglet, setOnglet] = useState<SousOnglet>('contacts')
  ...
  async function discuter(contact: Contact) {
    ...
    try {
      await onOuvrirChat(contact.user_id)
    } catch (e) {
  ...
  const tabBtn = (id: SousOnglet, label: string, count: number, badge = false) => (
    <button
      onClick={() => setOnglet(id)}
      className={`relative pb-2 px-0.5 text-[13px] font-semibold transition-colors ${
    ...
    <div className="flex flex-col h-full">
      <div className="px-4 pt-3">
        <button
          ...
          className="w-full h-10 rounded-xl bg-orange text-white flex items-center justify-center gap-2 text-sm font-semibold hover:bg-orange-hover transition-colors shadow-sm shadow-orange/30"
        >
            ...
            {loadingContacts ? (
              ...
              <ul className="space-y-2">
                {contactsFiltres.map((c) => (
                  <li key={c.user_id} className="flex items-center gap-3 bg-white border border-gray-100 rounded-xl px-3 py-2.5">
                    <Avatar nom={c.nom} />
        ...
        {onglet === 'demandes' && (
          <div>
            {loadingDemandes ? (
              ...
              <ul className="space-y-2">
                {demandes.map((d) => (
                  <li key={d.relation_id} className="bg-white border border-gray-100 rounded-xl px-3 py-2.5">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <button
                        ...
                        disabled={busy === d.relation_id}
                        className="h-9 px-3 rounded-lg bg-gray-100 text-gray-600 text-sm font-semibold hover:bg-gray-200 transition-colors disabled:opacity-40"
                      >
                        Refuser
                      </button>
                      <button
        ...
        {onglet === 'envoyees' && (
          <div>
            {loadingInv ? (
              ...
              <ul className="space-y-2">
                {invitations.map((inv) => (
                  <li key={inv.relation_id} className="flex items-center gap-3 bg-white border border-gray-100 rounded-xl px-3 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-400 truncate">
                        {inv.deja_inscrit ? 'Membre Nexartis · en attente' : 'Invité par email · en attente'}
                      </div>
                    </div>
      ...
      {showAjout && (
        <AjoutModal onClose={() => setShowAjout(false)} onDone={() => { refetchInv() }} />
      )}
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 bg-navy/40 flex items-end md:items-center justify-center p-0 md:p-4" onClick={onClose}>
      <div className="bg-white w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <h2 className="font-bold text-navy font-hanken">Ajouter un confrère</h2>

...
export default function PhotoSlot({ refData, label, jobs, getPreview, photosById, onPick, onRemove, onRetry, onRotate, big }: {
  refData: PhotoRef
  label?: string
    ...
    <div>
      ...
      <div className={`relative rounded-xl border border-gray-200 bg-gray-50 overflow-hidden ${big ? 'h-44' : 'h-32'}`}>
        {hasPhoto ? (
          ...
          <img src={src as string} alt={refData.legende || 'Photo du rapport'} className="w-full h-full object-contain transition-transform" style={{ transform: `rotate(${rot}deg)` }} />
        ) : (
        ...
        {hasPhoto && (
          <div className="absolute top-1.5 right-1.5 flex gap-1">
            {onRotate && (
              <button type="button" aria-label="Pivoter la photo" onClick={onRotate}
                className="w-7 h-7 rounded-lg bg-white/90 flex items-center justify-center text-navy hover:bg-white shadow"><RotateCw size={14} /></button>
            )}
            <button type="button" aria-label="Retirer la photo" onClick={onRemove}
              className="w-7 h-7 rounded-lg bg-white/90 flex items-center justify-center text-red-600 hover:bg-white shadow"><X size={15} /></button>
          </div>
        )}

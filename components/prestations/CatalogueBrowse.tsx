...
// ---------------------------------------------------------------------------
// CatalogueBrowse — consultation du catalogue de prestations par metier.
// Code extrait FIDELEMENT de app/dashboard/catalogue/page.tsx (selecteur metier
// + recherche + liste groupee par categorie, donnees statiques lib/catalogue).
//
// Ajout : chaque item gagne un bouton « + Ajouter a mes prestations » qui
// appelle la prop `onAdd(item)`. La page parente ouvre alors le modal de
// creation du mode « Mes prestations » pre-rempli (designation / unite / tva ;
// prix laisse vide a l'artisan).
// ---------------------------------------------------------------------------

import { useEffect, useMemo, useState } from 'react'
import { Library, Search, Info, Plus } from 'lucide-react'
...
export default function CatalogueBrowse({ onAdd }: { onAdd?: (item: CatalogueItem) => void }) {
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-navy/65 max-w-2xl">
        Des prestations prêtes par métier. Choisis-en une, ajoute-la à tes prestations
        et mets ton prix : elle sera ensuite proposée en autocomplétion quand tu rédiges un devis.
      </p>
      ...
      <p className="flex gap-2 text-sm text-navy/70 bg-sky/10 border border-sky/30 rounded-xl px-3 py-2.5">
        ...
        <span>
          Le catalogue est une vitrine : utilise «&nbsp;+ Ajouter&nbsp;» pour copier une prestation
          dans «&nbsp;Mes prestations&nbsp;», puis indique ton prix.
        </span>
      </p>
                ...
                {items.map((it) => (
                  ...
                  >
                    ...
                    {onAdd && (
                      <button
                        onClick={() => onAdd(it)}
                        className="shrink-0 inline-flex items-center gap-1 h-8 px-2.5 rounded-lg border-2 border-orange/40 text-orange text-xs font-semibold hover:bg-orange hover:text-white transition-colors"
                        aria-label={`Ajouter ${it.designation} à mes prestations`}
                      >
                        <Plus size={13} />
                        Ajouter
                      </button>
